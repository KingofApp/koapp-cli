...
function add(debug, pluginType, pluginName) {
  console.log('Start add');
  debugMode = debug;
  var validType = /^(module|theme|spinner|service)$/.test(pluginType);
  if(!pluginType || !validType) return tools.debugLog('Plugin type should be "module", "service", "spinner" or "theme"', debugMode);
  ...
  jsonfile.readFile('./com.kingofapp.visualizer/www/core/structure.json', function(err, obj) {
    if(err) return tools.debugLog(chalk.red('[ERROR] Reading structure.json. Call "koapp -command-" from the folder initialized by "koapp init" that contains com.kingofapp.visualizer') , debugMode);
    var newName = cleanPluginName(packageName);
    if (checkIfExists(pluginName, pluginType)) {
      console.log('Check if exists');
      buildPlugin(pluginType, newName, obj, 'add');
    } else {
      console.log('Read file success');
      shell.cd('./com.kingofapp.visualizer/www/' + pluginType + 's');
...
function buildPlugin(pluginType, newName, structure, method, userInput) {
  koappAction = method;
...
function dumpStructureAndBowerInstall(packageName, pluginType, structure) {
  ...
  if (pluginType === 'theme' || pluginType === 'spinner') {
    pluginFolder = pluginType + 's/koapp-' + pluginType + '-' + packageName;
  } else {
    pluginFolder = pluginType + 's/' + packageName;
  }

  jsonfile.writeFile('./com.kingofapp.visualizer/www/core/structure.json', structure, {spaces:2}, function(err, data) {
    if(err) return tools.debugLog(chalk.red('[ERROR] ' + err), debugMode);

    if (koappAction === 'add' && (pluginType === 'module' || pluginType === 'service')) {
      shell.cd('./com.kingofapp.visualizer/www/' + pluginFolder);
      shell.exec('bower install');
    }
    if (koappAction === 'add') tools.debugLog(chalk.green(packageName + ' has been downloaded and installed'), debugMode);
  });
...
function getModuleConfig(packageName, pluginType, userInput) {
  return new Promise(function(resolve, reject) {
    shell.cd('../../..');
    var config = jsonfile.readFileSync('./com.kingofapp.visualizer/www/' + pluginType + 's/' + packageName + '/config.json');
    [
      'description',
      'documentation',
      'downloads',

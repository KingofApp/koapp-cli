...
function build(debug, projectName, platform, options) {
  ...
  jsonfile.readFile(commandData.repo + '/www/core/structure.json', function(err, obj) {
    if (err) tools.debugLog('Error reading file structure.json');
...
function addSplashAndIcon(projectName, callback) {
  tools.debugLog(chalk.red("Generating icon and splash"), debugMode);
  ...
  async.parallel([
    ...
    if (err) {
      tools.debugLog('addSplashAndIcon' + err); return callback(err);
    }
...
function addCordovaPreferences(callback) {
  tools.debugLog(chalk.red("Adding cordova Dependencies..."), debugMode);
...
function addPlatform(platform, callback) {
  tools.debugLog(chalk.red('Adding platform...'), debugMode);

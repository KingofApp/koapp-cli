...
function create(debug, pluginType) {
  ...
  inquirer.prompt(prompt).then(function(answers) {
    var generate = 'cd com.kingofapp.visualizer/www/' + pluginType + 's && yo koapp-' + pluginType + ' ' + answers.pluginName;
    ...
    shell.exec(generate, function(err, data) {
      ...
      jsonfile.readFile('./com.kingofapp.visualizer/www/core/structure.json', function(err, obj) {
        ...
        addHelpers.buildPlugin(pluginType, answers.pluginName, obj, 'create');
      });

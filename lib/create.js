...
function create(debug, pluginType, env) {
  ...
  visualizerFolder = dev ? 'com.kingofapp.visualizer.dev' : 'com.kingofapp.visualizer';
  ...
  inquirer.prompt(prompt).then(function(answers) {
    ...
    var workingDirectory = upath.normalize(shell.pwd().stdout);
    if (workingDirectory.indexOf(`/${visualizerFolder}/www/${pluginType}s`) === -1) {
      return tools.debugLog(chalk.red(`\nError ${workingDirectory} finding the folder "${pluginType}s" inside your project. Call "koapp create" from the folder initialized by "koapp init" that contains "${visualizerFolder}"\n`), debugMode);
    }
...
function buildThemeOrSpinner(name, type, structure){
  structure.config[type].identifier = name.toLowerCase();
  structure.config[type].path = `${type}s/koapp-${type}-${name.toLowerCase()}/koapp-${type}-${name.toLowerCase()}.html`;

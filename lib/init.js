...
function init(debug, projectName, options) {
  var dev = (options && (options.indexOf('dev') >= 0 || options.indexOf('--dev') >= 0));
  ...
  shell.exec(clone, function(err, data) {
    ...
    if (options.indexOf("--no-deps") !== -1) {
      tools.debugLog(chalk.green('Koapp project has been created.'), debugMode);
    } else {
      tools.debugLog(chalk.red('Installing npm dependencies.'), debugMode);
      shell.exec('npm install && bower install', function(err) {
        if(err) return tools.debugLog(chalk.red('Error installing npm dependencies'), debugMode);

        shell.exec('bower install', function(err) {
          if (err) return tools.debugLog(chalk.red('Error installing bower dependencies.'), debugMode);

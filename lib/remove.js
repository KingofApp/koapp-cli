...
function remove(debug, pluginType, pluginId) {
  debugMode = debug;
  if(!checkPlugin(pluginType)) return tools.debugLog(chalk.red('Plugin must be "module" or "service"'), debugMode);
  tools.debugLog('Trying to remove ' + pluginId, debugMode);
  jsonfile.readFile('./com.kingofapp.visualizer/www//core/structure.json', function(err, obj) {
    var plugin = obj[pluginType + 's'];
    if(err)     return tools.debugLog(chalk.red(err), debugMode);
    if(!plugin) return tools.debugLog(chalk.red(pluginType + ' not found'), debugMode);

    for (var key in plugin) {
      if (plugin[key].identifier === pluginId) {
        tools.debugLog('deleted');
    ...
    jsonfile.writeFile('./com.kingofapp.visualizer/www/core/structure.json', obj, {spaces: 2}, function(err, data) {
...
function removeDeps(app, pluginType, pluginId) {
  tools.debugLog('Looking for dependencies:', debugMode);
  if (pluginType === 'module') {
    tools.debugLog(' - Modules', debugMode);
  ...
  if(app.services){
    tools.debugLog(' - Services', debugMode);
    removeDepsInServices(app, pluginType, pluginId);
  }
}

function removeFolder(pluginType, pluginId) {
  var folder = './com.kingofapp.visualizer/www/' + pluginType + 's';

  shell.cd(folder);
  tools.debugLog(pluginId);
...
function removeInMenus(app, paths) {
  tools.debugLog('   - Menus', debugMode);
...
function removeDepsInModules(app, pluginId) {
  tools.debugLog('   - Requires', debugMode);
...
function help() {
  var command = chalk.red('tools ') + chalk.green('remove ') + chalk.blue('<pluginType> <pluginId>');
  tools.debugLog(command, debugMode);
  tools.debugLog(' ', debugMode);
  tools.debugLog('  Remove plugin module or service from your app.', debugMode);
  tools.debugLog(' ', debugMode);
}
